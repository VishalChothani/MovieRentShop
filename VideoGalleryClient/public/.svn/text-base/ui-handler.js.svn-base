...
$(document).on('click', '#save_member_info', function(e) {
    ...
    var data = {};
    data[USER_LOGIN_ID] = user_login_id.value;
...
function loadEditMemberInfo() {
    $.ajax({
        ...
        $.ajax({
            ...
            $("#main-content").load( "UserAccount.ejs", {data : stateMembershipRes, selection : person[CODE], membership_selection : person[MEMBERSHIP_TYPE_ID]},
            		function(res, status, xhr) {
            	
            	user_login_id.value = person[ID];
...
$(document).on('click', '#payment_done', function(e) {
	...
	// Payment screen validations
	if(
			validateBlank(credit_card_number, 'Please Enter Card Number') &&
			validateNum(credit_card_number) &&
			validateLength(credit_card_number, 16, 'Card number should contain 16 digits') &&
			
			validateBlank(payment_name, 'Please Enter Name on Card') &&
			validateDropDown(card_month, 'month') &&
			validateDropDown(card_year, 'year') && 
			
			validateBlank(cvv, 'Please Enter CSC Number') &&
			validateNum(cvv) &&
			validateLength(cvv, 3, 'CSC number should contain 3 digits')) {
...
$(document).on('click', '#signupButton', function(e) {
    ...
    if(
    		...
    		validateEmail(email, 'Please Enter Email Id') &&
    		validateBlank(password, 'Please Enter Password') &&
    		validateBlank(ssn, 'Please Enter SSN') &&
    		validateSSN(ssn.value) &&
    		validateBlank(street, 'Please Enter Street') && 
    		validateBlank(city, 'Please Enter City') &&
    		validateDropDown(membership_type, 'Membership Type') &&
    		validateDropDown(state, 'State') && 
    		validateBlank(zipcode1, 'Please Enter ZIP') &&
    		validateZIP(zip)
		) {
