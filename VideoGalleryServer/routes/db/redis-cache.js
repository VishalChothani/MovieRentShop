...
RedisCache.cacheAllPersons = function(allPersonsList) {
	client.set("all_person_info", JSON.stringify(allPersonsList));
};

RedisCache.getCacheAllPersons = function(callback) {
	client.get("all_person_info", function (err, reply){
		callback(err,JSON.parse(reply));
	});
}; 
