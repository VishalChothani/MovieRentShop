...
exports.listAllPersons = function(req, res) {
	...
	RedisCache.getCacheAllPersons(function(err,allPersonsList){
		//console.log(allPersonsList);
		//var allPersonsList = {};
		//allPersonsList[Constants.PERSONS] = allPersonsList;
		utility.writeSuccessJSON(res, allPersonsList, "Details fetched successfully");
	});

	/*
	db.execSQL(Person.getListAllPersonsQuery(), function(err,result){
		var allPersonsList = {};
...
exports.editPersonInfo = function(req, res) {
	...
	if(!errors[Constants.STATUS]) {
		...
		db.update(userLogin.getUpdateUserLoginQuery(), userLogin.getUpdateUserLoginValuesMap(), function(err, result){
			if(!err) {
				db.update(p.getUpdatePersonInfoQuery(), p.getUpdateValuesMap(), function(){
					utility.writeSuccessJSON(res, p.toJSON(), 'Details updated successfully');
				});
			} else {
				var errorMessage;
				if(err.message.indexOf("ER_DUP_ENTRY") > -1) {
